...
- Claude Code는 `skills/` 바로 아래 1단계 폴더의 SKILL.md만 로드하므로, 스킬 추가 후 `scripts/local-skills-link/local-skills-link.sh`를 실행해 최상위 심링크(`skills/foo -> .ignore/foo`)를 만들어야 `foo`로 호출됩니다. 심링크는 `.git/info/exclude`에 자동 등록되어 git에 노출되지 않습니다
- 기기 간 동기화되지 않으므로 필요한 각 기기에서 직접 채우고 위 스크립트를 실행해야 합니다
