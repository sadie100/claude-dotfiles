...
| `migrate-to-shoehorn` | 테스트 `as` → shoehorn 마이그레이션 시 | mattpocock/skills | 테스트 파일의 `as` 단언을 @total-typescript/shoehorn으로 치환 |
...
| 스킬 | 트리거 | 소속 | 설명 |
|------|--------|------|------|
| `obsidian-vault` | Obsidian 노트 검색/생성/관리 시 | custom | 위키링크와 인덱스 노트로 Obsidian 볼트 노트 검색·생성·정리 |
...
| `setup-pre-commit` | 프리커밋 훅 셋업 시 | mattpocock/skills | Husky + lint-staged(Prettier)·타입체크·테스트 프리커밋 훅 설정 |
...
| `qa` | `/qa` | gstack | 웹앱을 체계적으로 QA 테스트하고 발견한 버그를 수정 |
...
| `chrome-devtools` | stdio | `npx chrome-devtools-mcp@1.3.0` | Chrome 브라우저 제어/디버깅/자동화 (chrome-devtools-mcp 플러그인 번들) |
