...
if ($ProfileContent -match "function dotclaude") {
    ...
    $ProfileContent = ($newLines -join "`n").TrimEnd() + "`n"
    Set-Content -Path $ProfilePath -Value $ProfileContent
    Write-Host "[clean]  Removed old inline dotclaude function from $ProfilePath"
}

# Remove old source block if present, then re-add
$ProfileContent = Get-Content $ProfilePath -Raw -ErrorAction SilentlyContinue
if ($ProfileContent -match [regex]::Escape($SourceMarker)) {
    $ProfileContent = $ProfileContent -replace "(?ms)$([regex]::Escape($SourceMarker)).*?# dotclaude-end\r?\n?", ""
