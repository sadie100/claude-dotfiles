...
| [`unlink/`](unlink/README.md) | `install`의 역방향 — symlink를 실제 복사본으로 바꿔 `~/.claude`를 레포에서 분리 |
...
`mcp-sync/`는 `dotclaude mcp pull|sync` 서브커맨드, `hooks/`는 `settings.json`의 훅 정의에서 각각 직접 호출된다. `unlink/`는 `install/*`의 역방향으로, `dotclaude unlink` 서브커맨드에서 호출된다.
