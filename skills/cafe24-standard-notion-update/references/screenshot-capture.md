...
5. 한 번 삽입한 이미지 블록은 MCP로 다시 지정할 수 없다(서명 URL 불일치 — SKILL.md §5). 교체·이동이 필요하면 새로 업로드해 넣고, 남는 옛 이미지는 사용자에게 위치를 알려 UI에서 지우게 한다. 원본이 필요하면 fetch 직후 서명 URL로 다운로드해 두면 재업로드에 쓸 수 있다(유효 5분).
