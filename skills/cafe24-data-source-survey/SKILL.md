...
| 원데이원볼 | https://oneday1ball.com/ | `ftp/oneday1ball/` |
| 엑쎄라피 | https://exerapy.co.kr/ | `ftp/exerapy/` (`exerapy-cafe24/`는 구버전 일부 파일만) |
...
- **기본 모듈 숨김 관례**: `product_detaildesign` 같은 기본 모듈이 렌더는 되는데 CSS/인라인으로 숨겨져 있고 실노출은 다른 소스인 몰이 많다. "보이는 것"과 "존재하는 것"을 구분해 기록한다. 숨김 CSS가 해당 기능 파일이 아니라 무관한 전역 오버라이드 파일에 있을 수 있으니, 노출 여부는 파일 추적이 아니라 렌더된 화면에서 판정한다.
- **앱스크립트 주입형 앱은 FTP 무흔적**: 카페24 앱스크립트 SDK(`CAFE24.APPSCRIPT_SDK_DATA`)로 주입되는 제3자 앱은 스킨 소스에 로드 코드가 전혀 없고, 위젯 DOM도 클라이언트 렌더라 원문 fetch에 안 잡힌다. 미러 전수 검색+원문 실측만으로 "앱 없음 = 네이티브"를 단정하지 말고, 브라우저에서 렌더된 화면과 로드 스크립트 목록까지 확인한다(엑쎄라피 스냅리뷰 오판 사례, 2026-07-27 — 상세는 `cafe24-analyze-plugin`의 3중 확인 참조).
