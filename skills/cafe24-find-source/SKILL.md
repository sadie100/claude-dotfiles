...
- **테스트 스킨(`skin-skinNN`) 프리뷰를 보고 있다면 raw 경로에도 그 프리픽스를 붙인다**
  (`/skin-skin50/js/...`). 프리픽스 없는 루트 상대 경로는 그 도메인의 **기본 적용 스킨**으로 풀린다 — 아래 주의사항 참고.
- 히트한 파일 + 라인이 **정답**이다.
...
- **테스트 스킨 raw fetch에는 스킨 프리픽스 필수:** `skin-skinNN` 프리뷰 페이지에서 `/js/module/...`처럼
  프리픽스 없이 fetch하면 카페24는 프리뷰 스킨이 아니라 **그 도메인의 기본 적용 스킨** 파일을 돌려준다.
  404가 아니라 200으로 멀쩡한 구버전이 와서 라인 번호까지 그럴듯하게 맞을 수 있다(실측: skin50 신버전 대신
  기본 스킨 구버전을 받고 원본 확정으로 오판). 페이지가 번들을 `/skin-skinNN/ind-script/...`로 로드하고 있다면
  raw fetch도 반드시 `/skin-skinNN/js/module/...`로. 번들 본문과 raw 본문이 다르면 캐시가 아니라 이 함정부터 의심한다.
