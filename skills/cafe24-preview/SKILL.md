...
4. **reload → 검증.** 마커 규칙(`--preview-marker` 등)이나 `getComputedStyle` 실측으로 패치 적용을 확인한 후 스크린샷/비교 진행.
5. **반복.** 라우트는 같은 page 인스턴스에 남아 **이후의 다른 MCP 도구 호출·새로고침에도 유지**된다. 수정 반복 시 `unrouteAll` 후 재설치.
6. **정리.** 검증 끝나면 `page.unrouteAll({ behavior: 'ignoreErrors' })`로 원상 복구.
